...
app.layout = html.Div([
    ...
    # Currency pair text input, within its own div.  # Style it so that the submit button appears beside the input.
    html.Div(["Input Currency:",html.Div(dcc.Input(id='currency-pair',value='AUDCAD',type='text'))], style={'display': 'inline-block'}),
    # Submit button:
    ...
    dcc.RadioItems(id='action', options=[{'label': 'BUY', 'value': 'BUY'}, {'label': 'SELL', 'value': 'SELL'}], value='BUY'),
    # Text input for the currency pair to be traded
    html.Div(["Trade Currency:", html.Div(dcc.Input(id='trade-currency',value='AUDCAD',type='text'))], style={'display': 'inline-block'}),
    # Numeric input for the trade amount
    html.Div(["Trade Amount:", html.Div(dcc.Input(id='trade_amt',value='60000',type='number'))], style={'display': 'inline-block'}),
    # Submit button for the trade
    html.Button('Trade',id='trade-button',n_clicks=0)])

# Callback for what to do when submit-button is pressed # there's more than one output here, so you have to use square brackets to pass it in as an array.
@app.callback([dash.dependencies.Output('output-div', 'children'), dash.dependencies.Output('candlestick-graph', 'figure')], dash.dependencies.Input('submit-button', 'n_clicks'), dash.dependencies.State('currency-pair', 'value'))
...
@app.callback(dash.dependencies.Output('trade-output', 'children'), dash.dependencies.Input('trade-button', 'n_clicks'), dash.dependencies.Input('action', 'value'), dash.dependencies.State('trade-currency', 'value'), dash.dependencies.State('trade_amt', 'value'), prevent_initial_call=True)
